...
var ServerManager = function(serverOptions) {
  ...
  // Add rest options
  if (serverOptions.rest === undefined) {
  ...
  var allowedOptions = ['sslOnNormalPorts', 'sslMode', 'sslPEMKeyFile',
    ...
    'sslCAFile', 'sslCRLFile', 'sslAllowConnectionsWithoutCertificates',
    ...
    'oplogSize', 'journalCommitInterval', 'journalOptions',
    'journal', 'notablescan', 'noscripting', 'repairpath', 'repair',
    'upgrade', 'syncdelay', 'quotaFiles', 'quota',
    'nssize', 'noIndexBuildRetry', 'directoryperdb',
    'dbpath', 'sysinfo', 'cpu', 'profile', 'slowms',
    ...
    setTimeout(function() {
      ...
      exec(bin + ' --version', function(error, stdout) {
        ...
        process = exec(cmd, function(error, stdout, stderr) {
          ...
          stdout.toString('utf-8').split(os.EOL).map(function(line) {
            debug('mongod:stdout > ', line);
          });
          stderr.toString('utf-8').split(os.EOL).map(function(line) {
